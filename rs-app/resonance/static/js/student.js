...
$('#submit').css("background-color","#848880");
    ...
    $.ajax({
            ...
            success: function(response){
                if (response.status != false) {
                    ...
                    $('#country-getting-started').append(`<option value="" >Select Country</option>`);
  ...
  function SelectState(){
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.status != false) {
                $('#state-getting-started').empty();
                $('#state-getting-started').append(`<option value="">Select State</option>`);
                $('#city-getting-started').empty();
                $('#city-getting-started').append(`<option value="" selected disabled>Select City</option>`);
                for(var i=0;i<response.data.length;i++){
                    $('#state-getting-started').append(`<option value="${response.data[i].id}">${response.data[i].label}</option>`);
...
function SelectCity(){
  ...
  $.ajax({
      ...
      success: function(response){
          if (response.status != false) {
              $('#city-getting-started').empty();
              $('#city-getting-started').append(`<option value="" >Select City</option>`);
              for(var i=0;i<response.data.length;i++){
                  $('#city-getting-started').append(`<option value="${response.data[i].id}">${response.data[i].label}</option>`);
                  //$('#city-getting-started').selectpicker('refresh');
              }
...
function SelectCenter(){
  var selected_state =$( "#city-getting-started" ).val();
  ...
  $.ajax({
      ...
      success: function(response){
          ...
          if (response.status != false) {
              $('#center-getting-started').empty();
              $('#center-getting-started').append(`<option value="">Select Center</option>`);

              for(var i=0;i<response.data.length;i++){
                 //alert(JSON.stringify(response.data[i]['name']))
                  $('#center-getting-started').append(`<option value="${response.data[i]['id']}">${response.data[i]['name']}</option>`);
                  //$('#center-getting-started').selectpicker('refresh');
              }
      }
  ...
  function attrOnChange(){
      var nameof=$('input[name="name"]'); 
      var emailof=$('input[name="student_email"]');
      var mobileof=$('input[name="student_mobile"]');
      var dobof=$('input[name="student_dob"]');
      var genderof=$('select[name="gender"]');
      var divisionof=$('select[name="division"]');
      var mediumof=$('select[name="medium"]');
      var countryof=$('select[name="country"]');
      var stateof=$('select[name="state"]');
      var cityof=$('select[name="city"]');
      var centerof=$('select[name="center"]');
      var rollnumberof=$('input[name="role_number"]');
      var sessionof=$('select[name="session"]');
      var programof=$('select[name="program"]');
      var phaseof=$('select[name="phase"]');
      var batchof=$('select[name="current_batch"]');
      var classof=$('select[name="class"]');
      console.log("changed");
    // if(){
    //   $('#submit').css("background-color","rgb(185, 211, 76)");
    // }
  }     
  ...
  jQuery(document).ready(function(){
    ...
    $('#failDialogue').css("display","flex");
     ...
     $('input[name="name"]').change(attrOnChange());
     $('input[name="student_email"]');
     $('input[name="student_mobile"]');
     $('input[name="student_dob"]');
     $('select[name="gender"]');
     ...
     $('select[name="city"]');
     $('select[name="center"]');
     $('input[name="role_number"]');
     $('select[name="session"]');
     $('select[name="program"]');
     $('select[name="phase"]');
     $('select[name="current_batch"]');
     $('select[name="class"]');
